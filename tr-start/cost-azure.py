...
def lambda_handler(event, context):
    ...
    try : 
        ...
        # Bedrock으로 비용 계산
        prompt = f"""
        You are a cloud financial analyst specializing in **Microsoft Azure cost estimation** from **Terraform configurations**.

        Your task:
        - Read and understand the Terraform configuration below.
        - Identify each Azure resource (e.g., Virtual Machine, Storage Account, App Service, SQL Database, Load Balancer, Network, Function App, etc.).
        - Estimate the **total monthly cost in USD** based on **typical pay-as-you-go pricing** in the **East US** region.
        - Include all major cost factors such as compute, storage, networking, and data transfer.

        ### Output Format
        - Output only **one numeric value** (e.g., `132.78`)
